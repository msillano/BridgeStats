...
public class BridgeProbs implements IGame
       ...
       static   DecimalFormat formN = new DecimalFormat("###.###");
         ...
         */
        public Object runGame(int action, String sParam)
                {
        ...
        public Object printGame(int action, String sParam)
                {
        	if (action == 1){
        		 BridgeCardSet useCards ;
                long n = getMAXHAND(3);
...
//                MyIO.stampa("Verifica:  totale = " + tot);
         ...
         *
         * @param handNS Stringa con le carte in NS (un char per carta)
         * @param map Array con il risultato (-1 se non usato).
         *
         * @return Numero dei valori posseduti da NS
         */
        private int remapFree1(String handNS, int[] map)
                {
                int count = 0;
                // inizializza a -1
                for (int i = 0; i < map.length;i++)
                        map[i] = -1;
                // Per ogni carta in handNS
                for (int i = 0; i < handNS.length(); i ++) {
                        int x = suite.getVIXCard( handNS.substring(i, i + 1));
